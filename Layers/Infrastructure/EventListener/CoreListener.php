...
abstract class CoreListener extends abstractListener
{
    ...
    final protected function _Heritage_roles(LifecycleEventArgs $eventArgs)
    {
        ...
        if ($this->tokenStorage->isUsernamePasswordToken()
            ...
            if ($this->container->get('sfynx.auth.role.factory')->setJsonFileRoles(true)) {
    ...
    /**
     * We remove json file Etag of Page.
     *
...
//             exit;
    ...
    /**
     * We find all template names of an entity and these nodes.
     *
     * @param LifecycleEventArgs $eventArgs   The LifecycleEventArgs class
                ...
                if (($plugin == "gedmo")
                    ...
                    try {
                        ...
                        if (is_object($entity_widget)
                            ...
                            //&& !($entity_widget->getPage() instanceof \Sfynx\CmfBundle\Layers\Domain\Entity\Page)
    ...
    final protected function _Delete_Permission_Page_ByUser(LifecycleEventArgs $eventArgs){
        ...
        // If  autentication user and the user has the permission to delete a page
        if ($this->tokenStorage->isUsernamePasswordToken()
                && (in_array('DELETE', $this->tokenStorage->getUserPermissions())
     ...
     * @author Etienne de Longeaux <etienne.delongeaux@gmail.com>
     */
    protected function _permission_management_Page_ByUser($eventArgs)
    {
        $entity        = $eventArgs->getEntity();
        $entityManager = $eventArgs->getEntityManager();
        ...
        if ($this->tokenStorage->isUsernamePasswordToken()
            ...
            if (!$this->isUserSatisfyByRolesEditors()
                && (
                    (
                        ($entity instanceof Page)
                        ...
                        && !$this->isUserSatisfyByRolesEditors()
                        && ($entity->getPage()->getUser() instanceof User)
                        && ($entity->getPage()->getUser()->getId() != $this->getToken()->getUser()->getId())
                    ) || (
                        ($entity instanceof Widget)
    ...
    protected function isUserSatisfyByRolesEditors()
    {
        return (in_array('ROLE_ADMIN', $this->tokenStorage->getUserRoles())
            || in_array('ROLE_SUPER_ADMIN', $this->tokenStorage->getUserRoles())
            || in_array('ROLE_CONTENT_MANAGER', $this->tokenStorage->getUserRoles())
        );
    }
    ...
    final protected function _Create_Block_Page(LifecycleEventArgs $eventArgs)
    {
        ...
        if ($this->tokenStorage->isUsernamePasswordToken()
            ...
            try {
                $ConfigXml = (string) $entity_page->getLayout()->getConfigXml();
                $layout_blocks = (new XmlCrawler($ConfigXml))->getSimpleXml();
            } catch (\Exception $e) {
                $layout_blocks = null;
            }

            if (($layout_blocks instanceof \SimpleXMLElement)
                && $layout_blocks->blocks->name->getName()
            ) {
                ...
                foreach ($layout_blocks->blocks->name as $key => $block_name) {
    ...
    /**
     * We register in the Symfony Cache the route of the page
     *
     * @param LifecycleEventArgs $eventArgs The LifecycleEventArgs class
     *
     * @return void
    ...
    final protected function _updateCacheUrlGenerator(LifecycleEventArgs $eventArgs)
    {
        ...
        } elseif ($this->tokenStorage->isUsernamePasswordToken()
                && ($entity instanceof TranslationPage) ) {
        } else {
            return false;
        }
        $routeCacheManager = $this->container->get('sfynx.tool.route.cache');
