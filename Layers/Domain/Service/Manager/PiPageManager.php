...
class PiPageManager extends PiCoreManager implements PiPageManagerBuilderInterface
{
    ...
    public function render($lang = '', $isSetPage = false)
    {
        ...
        if ($page instanceof Page) {
            ...
            if ($this->tokenStorage->isAnonymousToken()) {
        ...
        if ($page && $isEnabled_page) {
            $url_    = $this->container->get('request_stack')->getCurrentRequest()->getRequestUri();
            // Initialize response
            ...
            if ($pageTrans
                    && $pageTrans->getSecure()
                    && $this->tokenStorage->isAnonymousToken()
            ) {
                return $this->redirectHomePublicPage();
            }
            ...
            if ($page->getPublic() && $response->isNotModified($this->request->getCurrentRequest())) {
                ...
                // $response->setContent($this->container->get('twig')->render($this->renderSource($id, $this->language), array()));
...
//        exit;
    ...
    public function renderESISource($serviceName, $method, $id, $lang = '', $params = null, $options = null, Response $response = null)
    {
        $this->setLanguage($lang);
        // we initialize
        $this->initializeRequest($this->language, $options);
    ...
    public function initializeRequest($lang = '', array $options = array())
    {
        ...
        $clone_request = $this->request->duplicate();
...
//      $request = Request::createFromGlobals();  =>  $request = new Request($_GET, $_POST, array(), $_COOKIE, $_FILES, $_SERVER);
        ...
        $this->request->getAttributes()->set('_route', $match['_route']);
        $this->request->getAttributes()->set('_locale', $this->language);
        $this->request->setLocale($this->language);
        $_GET['_locale'] = $this->language;
    }
     ...
     * @param boolean $isSetPage True to force the registration page relationship
     * @param boolean $isForce True to force the setting cache page
     *
     * @return false|Page
     * @access public
     * @author Etienne de Longeaux <etienne.delongeaux@gmail.com>
     * @since  2012-01-23
     */
    public function setPageByRoute($route = '', $isSetPage = false, $isForce = false)
    {
        // Get corresponding page
        if (!$route || empty($route)) {
            $page = $this->getRepository('page')->getHomepage();
        } else {
            $page = $this->getPageByRoute($route, $isForce);
        }
        if ($page instanceof Page) {
            // we set the result
            $this->setCurrentPage($page);
     ...
     *
     * @param string  $route   Route page value
     * @param boolean $isForce True to force setting page in cache
     *
     * @return Page
     * @access public
     * @author Etienne de Longeaux <etienne.delongeaux@gmail.com>
    ...
    {
        if (!$isForce
            && $path_page_json_file = $this->isJsonPageFileExisted($route)
        ) {
            $page = unserialize(file_get_contents($path_page_json_file));
        } else {
    ...
    {
        $url_redirection = $this->container->get('sfynx.tool.route.factory')->generate($route_name);
     ...
     * @since  2012-01-23
     */
    protected function setTranslations(Page $page, $locale = false)
    {
        if (
            !isset($this->translations[$page->getId()])
            ||
            empty($this->translations[$page->getId()])
        ) {
            if (!$locale) {
                // records all translations
                $all_translations = $page->getTranslations();
                if ($all_translations->count() >= 1) {
                ...
                if ($translationPage instanceof \Sfynx\CmfBundle\Layers\Domain\Entity\TranslationPage) {
     ...
     * @since  2012-01-23
     */
    protected function setBlocks(Page $page)
    {
     ...
     * @param int $idpage id page
     *
     * @return array of \Sfynx\CmfBundle\Layers\Domain\Entity\Block
     * @access public
     * @author Etienne de Longeaux <etienne.delongeaux@gmail.com>
     * @since  2012-01-23
     */
     ...
     * @since  2012-01-31
     */
    protected function setWidgets(Page $page)
    {
        if (isset($this->blocks[$page->getId()]) && !empty($this->blocks[$page->getId()])) {
     ...
     * @since 2012-01-31
     */
    protected function setResponsePage(Page $page, Response $response)
    {
     ...
     * @since  2012-02-14
     */
    protected function redirectHomePublicPage(){
     ...
     * @since  2012-02-28
     */
    protected function getPagesByRub($idRubrique)
    {
        $pages_content   = "";
...
//                    print_r('<br />');print_r('<br />');
    ...
    public function cacheRefresh()
    {
        ...
        // we get the current page.
        $page = $this->getCurrentPage();
        if (!(null === $page) && !(null === $this->translations[$page->getId()])) {
            foreach ($this->translations[$page->getId()] as $translation) {
                ...
                $referer_url = str_replace($this->container->get('request_stack')->getCurrentRequest()->getUriForPath(''), '', $referer_url);
...
//                              print_r($this->container->get('request_stack')->getCurrentRequest()->getLocale());
    ...
    public function cachePage($entity, $type)
    {
        ...
        if ($entity instanceof Page) {
            ...
            if (in_array($type, array('persist', 'update'))) {
                ...
                $jsonContent = serialize($entity);
                PiFileManager::save($path_page_json_file, $jsonContent, null, LOCK_EX);
...
//                && method_exists($page, 'getLayout')
//            ) {
//                $new_page->setLayout($em->getReference('Sfynx\AuthBundle\Domain\Entity\Layout', $page->getLayout()->getId()));
//            }
//            $new_page->setPageCss(new ArrayCollection());
//            $new_page->setPageJs(new ArrayCollection());
//            $new_page->setUser(null);
//            if (!(null === $page->getUser())
//                && method_exists($page, 'getUser')
//            ) {
//                $new_page->setUser($em->getReference('Sfynx\AuthBundle\Domain\Entity\User', $page->getUser()->getId()));
//            }
//            $new_page->setRubrique(null);
//            if (!(null === $page->getRubrique())
//                && method_exists($page, 'getRubrique')
//            ) {
//                $new_page->setRubrique($em->getReference('Sfynx\CmfBundle\Layers\Domain\Entity\Rubrique', $page->getRubrique()->getId()));
//            }
            $new_page->setTranslations(new ArrayCollection());
            ...
            foreach ($this->translations[$id] as $translation) {
                ...
                $new_translation->setTags(new ArrayCollection());
                $new_translation->setLangCode(null);
                if (!(null === $translation->getLangCode())
                    && method_exists($translation, 'getLangCode')
                ) {
                    $new_translation->setLangCode($em->getReference('Sfynx\AuthBundle\Domain\Entity\Langue', $translation->getLangCode()->getId()));
                }
    ...
    public function getUrlByType($type, $entity = null)
    {
        ...
        switch ($type) {
            ...
            case 'widget':
                ...
                if (is_int($entity)) {
                    $entity = $this->getWidgetById($entity);
                }
                if ($entity instanceof Widget) {
                    ...
                    $Url['edit']      = $this->container->get('router')->generate('admin_homepage', array('NoLayout' => true));
                    $Url['import']    = $this->container->get('router')->generate('public_importmanagement_widget', array('id_widget' => $entity->getId(), 'NoLayout' => true));
                    try {
                        $xmlConfig    = new \Zend_Config_Xml($entity->getConfigXml());
                        $widgetClass  = ucfirst(strtolower($entity->getAction())) . 'Handler';
                        ////////////////// url management of gedmo snippet ///////////////////////////
                        if (($entity->getPlugin() == "gedmo")
                            ...
                            if ($is_snippet && !empty($id_snippet)) {
                                ...
                                $xmlConfig   = new \Zend_Config_Xml($xmlConfig);
                                $Url['admin'] = $this->container->get('router')->generate('admin_widget_edit', ['id' => $id_snippet, 'NoLayout' => true]);
                            }
                        }
                        ...
                        if (($entity->getPlugin() == "gedmo")
                            ...
                            try {
                                $Lists    = call_user_func('\\Sfynx\\CmfBundle\\Layers\\Domain\\Service\\Util\\PiWidget\\Gedmo\\' . $widgetClass . '::getAvailable');
                            } catch (\Exception $e) {
                                $Lists = null;
                            }
                        ...
                        if (($entity->getPlugin() == "content")
                            && $xmlConfig->widgets->get('content')
                        ) {
                            if ( $xmlConfig->widgets->content->get('snippet') && $xmlConfig->widgets->content->get('id') ) {
                                $Url['edit'] = $this->container->get('router')->generate('admin_widget_edit', ['id' => $xmlConfig->widgets->content->get('id'), 'NoLayout' => true]);
                            }
                        }
                        ...
                        if (($entity->getPlugin() == "content")
                            ...
                            try {
                                $Lists    = call_user_func('\\Sfynx\\CmfBundle\\Util\\PiWidget\\Content\\' . $widgetClass . '::getAvailable');
                            } catch (\Exception $e) {
                                $Lists = null;
                            }
                            $params['NoLayout'] = true;
