...
class PiLuceneManager extends PiCoreManager implements PiSearchLuceneManagerBuilderInterface
{
     ...
     * @var array
     */
    public static $_delete_tags = ['searchlucene', 'footer'];
    ...
    public function __construct(
        ...
        if (PiFileManager::mkdirr($dossier, 0755)) {
    ...
    public function indexPage(Page $page)
    {
        ...
        if ($translationPage instanceof \Sfynx\CmfBundle\Layers\Domain\Entity\TranslationPage) {
            ...
            $pathInfo     = str_replace($this->request->getUriForPath(''), '', $this->container->get('request_stack')->getCurrentRequest()->headers->get('referer'));
            ...
            $indexValues['Contents']    = $this->deleteTags(file_get_contents($this->request->getUriForPath('') . $pathInfo));
     ...
     * @since 2012-06-11
     */
    public function deletePage(\Sfynx\CmfBundle\Layers\Domain\Entity\Page $page)
    {
        // Open the index
        self::open($this->_indexPath);
    ...
    public function contentPage($pathInfo, $Query = null, $MaxResultByWord = 5, $class = "", $MaxLimitCara = 0)
    {
        ...
        try {
            ...
            $body = file_get_contents($this->container->get('request_stack')->getCurrentRequest()->getUriForPath('') . $pathInfo);
