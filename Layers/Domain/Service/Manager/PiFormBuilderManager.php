...
use Sfynx\CmfBundle\Layers\Domain\Service\Manager\Generalisation\Interfaces\PiFormBuilderManagerInterface;
...
class PiFormBuilderManager extends AbstractType implements PiFormBuilderManagerInterface
{
     ...
     * @static
     */
    protected static $_types_form = ['zend', 'symfony'];
    ...
    public function __construct(
        ...
        $actionName = '',
        $type_form = '',
        $form_template = 'model_form_builder.html.twig',
        $form_name = "myform"
    ) {
        $this->container      = $containerService;
        $this->_em            = $this->container->get('doctrine')->getManager();
        $this->_locale        = $this->container->get('request_stack')->getCurrentRequest()->getLocale();
        $this->_id_widget     = $this->container->get('request_stack')->getCurrentRequest()->get('id_widget', null);
        $this->_id_block      = $this->container->get('request_stack')->getCurrentRequest()->get('id_block', null);
        $this->_id_form       = $this->container->get('request_stack')->getCurrentRequest()->get('id_form', null);
...
//                                        ),
     ...
     * @since 2012-06-26
     */
    protected function setPerformControls()
    {
        // Set configuration
        $this->setConfig();
    ...
    protected function renderForm()
    {
        if ($this->getTypeForm() == "zend") {
            ...
            $response = $this->container->get('templating')->renderResponse($this->container->getParameter('sfynx.template.theme.layout.admin.form.builder').":{$this->_form_template}", array(
     ...
     * @since 2012-06-26
     */
    protected function setFormByControls()
    {
        //set instance
        $Allgroup = [];
...
//         $this->_form->addElement($instanceElement);
     ...
     * @since 2012-09-11
     */
    protected function setControlAction()
    {
        ...
        if ( ($this->isSubmitted()) ) {
            ...
            $this->actionSubmit();
            // Redirection
            $new_url     = $this->container->get('sfynx.tool.route.factory')->generate();
    ...
    protected function isSubmitted()
    {
        if ($this->getName() == $this->_id_form){
            if ($this->getTypeForm() == "zend"){
                $form_instance = $this->container->get('request_stack')->getCurrentRequest()->query->get('form_instance');
                if ($form_instance) {
     ...
     * @since 2012-06-26
     */
    protected function getPerformName()
    {
        ...
        if ($this->getName() == $this->_id_form) {
            ...
            try {
                ...
                if (is_array($XmlConfigWidget) && (count($XmlConfigWidget)>=1)) {
                    Data2XMLTransformer::init('1.0', 'UTF-8');
                    $this->_xmlconfig = Data2XMLTransformer::build('config', $XmlConfigWidget['xml'])->saveXML();
                }
            } catch (\Exception $e) {}
            // we create/update a widget if the xml config value is defined.
            try {
                if (!(null === $this->_xmlconfig) && $this->_performName == "insert") {
                    $this->insertWidget($XmlConfigWidget['plugin'], $XmlConfigWidget['action']);
                } elseif (!(null === $this->_xmlconfig) && $this->_performName == "update") {
     ...
     * @since 2012-09-11
     */
    protected function is_Index_exist()
    {
        if ((null === $this->_id_widget) || empty($this->_id_widget)) {
            return false;
        }
        return true;
    }
     ...
     * @since 2012-09-11
     */
    protected function insertWidget($plugin, $action, $message = "pi.session.flash.right.create")
    {
        try {
            $entity  = new \Sfynx\CmfBundle\Layers\Domain\Entity\Widget();
            $block   = $this->_em->getRepository('SfynxCmfBundle:Block')->find($this->_id_block);
            if ($block instanceof \Sfynx\CmfBundle\Layers\Domain\Entity\BLock) {
     ...
     * @since 2012-09-11
     */
    protected function updateWidget($plugin, $action, $message = "pi.session.flash.right.update")
    {
