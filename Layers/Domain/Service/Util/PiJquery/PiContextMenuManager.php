...
class PiContextMenuManager extends PiJqueryExtension
{
     ...
     * @static
     */
    static $thems = ['xp', 'default', 'vista', 'osx', 'human', 'gloss', 'gloss,gloss-cyan', 'gloss,gloss-semitransparent', 'pi', 'pi2'];

    /**
     * @var array
     * @static
     */
    static $menus = ['exemple', 'admin'];
    ...
    protected function render($options = null)
    {
        ...
        if (!isset($options['menu'])
            || empty($options['menu'])
            || (isset($options['menu']) && !in_array($options['menu'], self::$menus))
        ) {
            throw ExtensionException::optionValueNotSpecified('id', __CLASS__);
        }
        if (!isset($options['them'])
            || empty($options['them'])
            || (isset($options['them']) && !in_array($options['them'], self::$thems))
        ) {
            $options['them'] = 'human';
        }
