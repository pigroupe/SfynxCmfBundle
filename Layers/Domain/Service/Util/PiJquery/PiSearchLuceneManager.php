...
use Sfynx\CmfBundle\Layers\Domain\Service\Manager\PiLuceneManager;
...
class PiSearchLuceneManager extends PiJqueryExtension
{
     ...
     * @author Etienne de Longeaux <etienne_delongeaux@hotmail.com>
     */
    protected function renderpageAction($archive, $options = null)
    {
        ...
        $page           = intval($this->container->get('request_stack')->getCurrentRequest()->get('page'));
        $max            = intval($this->container->get('request_stack')->getCurrentRequest()->get('max', 5));
        $query_search   = $this->container->get('request_stack')->getCurrentRequest()->get('query_search');
        $count          = count($archive);
        $paginator      = array();
        $pagination     = null;
    ...
    public function searchpageMenu($options = null)
    {
        ...
        $query         = $this->container->get('request_stack')->getCurrentRequest()->query->get('query_search');
